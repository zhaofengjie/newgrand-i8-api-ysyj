...
import com.newgrand.domain.LM.SaveResultModel;
...
public class ZczrRecBackController {
    ...
    private JSONArray DetailForm(String projectId, JSONArray detailList) throws Exception {
        ...
        for (int i = 0; i < detailList.size(); i++) {
            ...
            String sql = "select ITEMDATA.phid,ITEMDATA.itemname from itemdata left join res_bs on ITEMDATA.PHID_RESBS = RES_BS.PHID  left join msunit on msunit.phid=itemdata.phid_msunit where ITEMDATA.itemno = '" + ItemNo + "'";
            var dataList = jdbcTemplate.queryForList(sql);
